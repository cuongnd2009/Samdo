...
(function($) {
    ...
    function Plugin(element, options) {
        ...
        function init() {
            ...
            $('#' + options.wrapperId).before('<a href="#" id="' + options.wrapperId + '-left-btn" class="d-flex align-items-center justify-content-center width-4 btn mt-1 mb-1 mr-2 ml-1 p-0 fs-xxl text-primary"><em class="fal fa-angle-left"></em></a>');
            $('#' + options.wrapperId).after('<a href="#" id="' + options.wrapperId + '-right-btn" class="d-flex align-items-center justify-content-center width-4 btn mt-1 mb-1 mr-1 ml-2 p-0 fs-xxl text-primary"><em class="fal fa-angle-right"></em></a>');
...
var initApp = (function(app) {
	 ...
	 * @return
	 **/
	app.saveSettings = function (reset = false) {
	...
	app.updateTheme = function (themeName, themeSave) {
	  const theme = themeName.charAt(0) === '/' ? themeName : ('/' + themeName);
		/* if theme anchor exists */
		if ($(myapp_config.mythemeAnchor).length) {
			$(myapp_config.mythemeAnchor).attr('href', theme);
		} else {
			$('head').append($("<link>", {id: myapp_config.mythemeAnchor.replace('#', ''), "rel": "stylesheet", "href" : theme }));
		}
    ...
    myapp_config.root_.addClass('mod-bg-1')
	...
	app.factoryReset = function () {
		...
		if (typeof bootbox  != 'undefined') {
			...
			bootbox.confirm({
				title: "<em class='" + myapp_config.appIconPrefix +  " fa-exclamation-triangle text-warning mr-2'></em> You are about to reset all of your localStorage settings",
	 ...
	 * @return {[type]}
	 **/
	app.domReadyMisc = function(myapp_config) {
				...
				switch ( true ) {
					...
					case ( actiontype === 'panel-close' ):
						...
						if (typeof bootbox  != 'undefined') {
							...
							bootbox.confirm({
								title: "<em class='" + myapp_config.appIconPrefix +  " fa-times-circle text-danger mr-2'></em> Do you wish to delete panel <span class='fw-500'>&nbsp;'" +selectedPanel.children('.panel-hdr').children('h2').text().trim()+ "'&nbsp;</span>?",
